...
$$V_P = V_G + V_M + V_E$$
Genetic and environmental factors contribute to maternal affects [@lynch_genetics_1998]. Although we attempted to minimize maternal effects by growing a refresher generation in a common greenhouse environment and *M. cardinalis* seeds are tiny and not well provisioned, seed traits may be particularly sensitive to maternal effects. We did not have statistical power to estimate nonadditive genetic contributions, therefore we estimated broad-sense ($H^2$) heritability, the fraction of phenotypic variance contributed by all sources of (including dominance and epistasis) genetic variance:
...
Using $\sigma_{\mathrm{parent}}$ rather than $\sigma_{\mathrm{sire}}$ is more powerful because it uses all of the data. We estimated $\sigma_{\mathrm{parent}}$ and other parameters using a Bayesian mixed effects model. We describe the general approach here and provide detail specific to germination rate and survival below. We fit the model in *Stan* version $`r stringr::str_c(fit[["metadata"]]()["stan_version_major"], fit[["metadata"]]()["stan_version_minor"], fit[["metadata"]]()["stan_version_patch"], sep = ".")`$ [@stan_development_team_stan_2022] using **cmdstanr** version $`r packageVersion("cmdstanr")`$ [@gabry_cmdstanr_2022]. *Stan* calculates the posterior distribution of the model using the Hamiltonian Monte Carlo algorithm, which is similar to the more widely used Markov Chain Monte Carlo, but is faster and more efficient at sampling for many applications [@monnahan_faster_2017]. We used weakly informative normally distributed priors for parameters that affect the trait mean (intercepts and coefficients) and half-Cauchy priors for variance parameters. Weakly informative priors are strongly recommended for complex mixed models [@mcelreath_statistical_2016]. We ran $`r length(fit$metadata()$id)`$ parallel chains for $`r fit[["metadata"]]()[["iter_warmup"]]`$ warmup iterations, $`r fit[["metadata"]]()[["iter_sampling"]]`$ sampling iterations, and a thinning interval of $`r fit[["metadata"]]()[["thin"]]`$. This configuration allowed parameters to converge, which we defined as the convergence diagnostic $\hat{R} < 1.05$ [@vehtari_rank-normalization_2021]. We inspected posterior predictive plots to assess model adequacy using the `pp_check` function from the *R* package **bayesplot** version $`r packageVersion("bayesplot")`$ [@gabry_visualization_2019]. We used the posterior median for point estimates and calculated uncertainty with the $95$\% quantile intervals from the posterior distribution.
...
Fewer plants survived over winter in the North garden than in the South garden (`r survival_north`% versus `r survival_south`%). In both gardens, southern source populations survived more often than northern source populations even after accounting for block effects (Fig. \ref{fig:mean-traits}b). For example, in the North garden, we estimated that plants originating from the most southern source population, Sweetwater River, survived `r p_CUR_north` of the time compared to `r p_RCK_north` for the most northern source population Rock Creek. Similarly, in the South garden the local population from Sweetwater River survived `r p_CUR_south` of the time compared to `r p_RCK_south` for Rock Creek. For most populations, the average temperature was warmer than the historical normal from $1961 \textendash 1990$, especially for the winter season in the South garden (Appendix S1, Fig. \ref{fig:climate}). However, temperatures for the North garden were close to their historical norm.

## Variance in germination rate and survival within and among populations
...
Data are available on Dryad at [https://doi.org/10.5061/dryad.18931zd0t](https://doi.org/10.5061/dryad.18931zd0t). Source code for all analyses is archived on Zenodo at [https://doi.org/10.5281/zenodo.6814728](https://doi.org/10.5281/zenodo.6814728). A repository with data and code is available on GitHub [https://github.com/cdmuir/mimulus-germination](https://github.com/cdmuir/mimulus-germination).
