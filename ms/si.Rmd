...
knitr::kable(df_pop_lm, format = "markdown", caption = "\\label{tab:df_pop_lm} Between-population directional selection coefficient estimates and confidence intervals. In both North and South gardens, we estimated selection on time to germination (log-mean scale) caused by increased winter survival ($\\text{logit}(p_\\text{surv})$). The parameters are the linear regression slope and intercept. Point estimates and 95\\% confidence intervals (CI) are estimated from the median and quantiles of the posterior distribution.", label = NA)
```
